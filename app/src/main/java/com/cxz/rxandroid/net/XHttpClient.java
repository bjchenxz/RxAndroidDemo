package com.cxz.rxandroid.net;

import android.net.Uri;

import com.cxz.rxandroid.net.interceptor.XLogInterceptor;
...
/**
 * XHttpClient for okhttp simple encapsulation
...
public class XHttpClient {
    ...
    private static final XHttpClient mHttpClient = new XHttpClient();
    private static final OkHttpClient mOkHttpClient = new OkHttpClient();

    static {
        mOkHttpClient.setConnectTimeout(5000, TimeUnit.MILLISECONDS);
        //添加日志过滤器
        XLogInterceptor xLogInterceptor = new XLogInterceptor(new XLogInterceptor.Logger(){
            @Override
            public void log(String message) {
                XLog.d(message);
            }
        });
        xLogInterceptor.setLevel(XLogInterceptor.Level.BODY);
        mOkHttpClient.interceptors().add(xLogInterceptor);
    }

    public static XHttpClient getHttpClient() {
        return mHttpClient;
    }
