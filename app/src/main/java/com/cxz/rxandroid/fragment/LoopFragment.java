...
public class LoopFragment extends RxFragment {
    ...
    private static final int[] DATAS = new int[]{R.drawable.pic_1,R.drawable.pic_2,R.drawable.pic_3,R.drawable.pic_4};
    ...
    void stopLoop(){
        if(subscription_auto != null && !subscription_auto.isUnsubscribed()){
            subscription_auto.unsubscribe();
        }
    }

    private void autoLoop(){
        if(subscription_auto == null || subscription_auto.isUnsubscribed()){
            subscription_auto = Observable.interval(3000,3000, TimeUnit.MILLISECONDS)
                                                //延时3000 ，每间隔3000，时间单位
    ...
    private class PicLoopAdapter extends PagerAdapter{
        ...
        public PicLoopAdapter(int[] mDatas) {
            this.mDatas = mDatas;
        }
